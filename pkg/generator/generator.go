...
func (g *GardenLinuxCloudInitGenerator) Generate(osc *generator.OperatingSystemConfig) ([]byte, *string, error) {
	...
	if osc.Object.Spec.Type == gardenlinux.OSTypeGardenLinux &&
		...
		nfBackendScript, nfBackendUnit, err := gardenlinuxgenerator.ConfigureNetFilterBackend(osc.Object, decoder)
		if err != nil {
			return nil, nil, err
		}
		...
		osc.Files = append(osc.Files, cgroupScript, lsmScript, nfBackendScript, restartScript)
		osc.Units = append(osc.Units, cgroupUnit, lsmUnit, nfBackendUnit, restartUnit)
		unitsToEnable = []string{cgroupUnit.Name, lsmUnit.Name, nfBackendUnit.Name, restartUnit.Name}
